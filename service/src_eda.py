...
# Вызываемые в сервисе функции
# Агрегированная статистика
def get_image_size(image_names, images_path, specific_images=None):
    ...
    for image_file in images_to_process:
        image_file = images_path / f"{image_file}.jpg"
        try:
...
# Heatmap данные
async def get_bboxes_heatmap(image_names, images_path, labels_path, class_names,
                             image_shape=(500, 500),  specific_classes=None):
    heatmap = np.zeros(image_shape, dtype=np.float32)

    for image_name in image_names:
        label_file = labels_path / f"{image_name}.txt"
        image_file = images_path / f"{image_name}.jpg"

        try:
            with Image.open(image_file) as img:
                img_width, img_height = img.size

            async with aiofiles.open(label_file, mode="r") as file:
...
# Функции визуализаци для фронта проекта
def get_grouped_hist(group_x_data, group_y_data, group_x_name, group_y_name, var_name, figsize=(7, 5), bins=10):
    fig, ax = plt.subplots(figsize=figsize)

    ax.hist(group_x_data, bins=bins, alpha=0.6, label=f'Image {var_name} {group_x_name}',
            color='blue', edgecolor='white', zorder=2)
    ax.hist(group_y_data, bins=bins, alpha=0.7, label=f'Image {var_name} {group_y_name}',
            color='forestgreen', edgecolor='white', zorder=2)
