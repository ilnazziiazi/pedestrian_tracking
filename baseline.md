...
Реализовали с нуля архитектуру YOLO v1 для понимания принципов её работы. Использовали PyTorch для создания и обучения модели.

## Архитектура модели

Модель состоит из сверточной части (darknet) и полносвязных слоев. Архитектура повторяет оригинальную статью с небольшими модификациями:
- Добавлен BatchNorm после сверточных слоев
- Изменено количество классов на 12 (в соответствии с датасетом)
- Уменьшен размер полносвязных слоев для ускорения обучения

Конфигурация сверточной части определена в architecture_config

## Функция потерь

Реализована оригинальная функция потерь YOLO, состоящая из нескольких компонентов:
- Потери координат bbox (с коэффициентом λ=5)
- Потери уверенности для объектов
- Потери уверенности для фона (с коэффициентом λ=0.5) 
- Потери классификации

Для расчета потерь используется MSE. Функция учитывает только один bbox с лучшим IoU из двух предсказанных.

## Обучение

Датасет разделен на train и test выборки. Для загрузки данных реализован кастомный датасет:
- Читает аннотации из csv файла
- Преобразует координаты bbox в формат YOLO (относительные координаты центра и размеры)
- Конвертирует разметку в целевую матрицу 7x7x(C+5*B)

## Оценка качества

Для оценки качества детекции используется mean Average Precision (mAP). 

## Результаты

Модель была обучена и провереа на датасете, используемом в оригинальной статье (VOC Dataset).

При попытке использовать выбранный датасет Aerial, модель не смогла обучиться. Возможно, это связано с тем, что в картинках имеется слишком много объектов, из-за которых могло произойти переполнение памяти.
